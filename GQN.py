...
class GQN_agent():
	def __init__(self, **kwargs):
		...
		self.actor = Actor(self.state_dim, self.action_dim, self.hidden_dim, self.action_bound, self.gmm_k).to(self.device)
		mu_params_to_optimize = [
			{'params': self.actor.fc1.parameters()},
            {'params': self.actor.fc2.parameters()},
            {'params': self.actor.fc_mu.parameters()}
        ]
		self.mu_optimizer = torch.optim.Adam(mu_params_to_optimize, lr=self.a_lr)
		lamda_params_to_optimize = [
			{'params': self.actor.fc1.parameters()},
            {'params': self.actor.fc2.parameters()},
            {'params': self.actor.fc_lamda.parameters()}
        ]
		self.lamda_optimizer = torch.optim.Adam(lamda_params_to_optimize, lr=self.a_lr)
	...
	def train(self):
		...
		with torch.no_grad():
			...
			smoothed_target_a = (self.actor_target(s_next) + target_a_noise).clamp(-self.action_bound, self.action_bound)
		...
		if self.delay_counter > self.delay_freq:
			# Update the Actor - mu
			mu_loss = -self.q_critic.Q1(s, self.actor(s)).mean()
			self.mu_optimizer.zero_grad()
			mu_loss.backward()
			self.mu_optimizer.step()

			# Update the Actor - lamda
			lamda_loss = -self.q_critic.Q1(s, self.actor(s)).mean()
